...
static uint8_t Console_Thread_stack[4096] BSP_PLACE_IN_SECTION(".stack.Console_Thread") BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);
...
void Console_Thread_create(void)
{
    ...
    UINT err;
    err = tx_thread_create (&Console_Thread, (CHAR*) "Console_Thread", Console_Thread_func, (ULONG) NULL,
                            &Console_Thread_stack, 4096, 4, 4, 50, TX_AUTO_START);
    if (TX_SUCCESS != err)
    {
