...
//extern TX_SEMAPHORE g_ob1203_semaphore;
...
void IRQ_Enable(void)
{
    ...
    if (FSP_SUCCESS != status)
    {
        APP_DBG_PRINT("**\n R_ICU_ExternalIrqEnable failed ** %u \r\n", status);
        APP_ERR_TRAP(status);
    }
    else
    {
        APP_PRINT("\nR_ICU_ExternalIrqEnable success %u \r\n", status);
    }
}
...
void ob1203_spo2_main(struct oxim *_ox, struct spo2 *_sp, struct ob1203 *_ob)
{
    ...
    { /*prox mode case*/
       ...
       // tx_semaphore_get(&g_ob1203_semaphore, TX_WAIT_FOREVER);
